...
enum Fingers2 {
    case thumb(_: Int) //=1
    case indexFinger(_: Int) //=2
    case middleFinger(_: Int) //=3
    case ringFinger(_: Int) //=4
    case pinky(_: Int) //=5
}
//: __Problem 3__
//:
...
struct Cookie {
    ...
    var delicious: Bool {
        get {
            return minutesSinceRemovalFromOven < 30
        }
    }
    
    func tempt() -> Void {
        if (delicious) {
            print("the \(flavor) flavored cookie is hot and just out of oven. It is so tempting")
        } else {
            print("the \(flavor) flavored cookie is cold and not so tempting")
        }
    }
}
//:__4b.__
...
var c1 = Cookie(flavor: "chocolate", minutesSinceRemovalFromOven: 12)
print(c1.tempt())
//: __Problem 5__
//:
...
class BnListing {
    ...
    init(type: Type, availability: Bool) {
        self.type = type
...
var bb = BnListing(type: Type.apartment, availability: false)
print(bb.reservation())
