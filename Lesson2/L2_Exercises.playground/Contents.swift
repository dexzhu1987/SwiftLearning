...
if let director = moviesDict[movie] {
    print("Movie : \(movie) is directed by \(director) " )
} else {
    print("Can not find any data related to this movie: \(movie)")
}


//: 1b) Test your code by inserting different values for the variable `movie`.
var movie2 = "Some Movie"
if let director = moviesDict[movie2] {
    print("The movie : \(movie2) is directed by \(director) " )
} else {
    print("Can not find any data related by this movie: \(movie2)")
}
//: ### Exercise 2
...
if let artist = paintingDict[painting]{
    print("\(painting) is created by \(artist)")
} else {
    print("Can't not find this painting")
}
