...
let yourSentence = "I have a \(nounArray[0]) and \(nounArray[4]) )"
let yourSillySentence = "The \(nounArray[index1]) is on top of \(nounArray[index2]) "
print(yourSentence)
...
var output: String = ""
var words = sillyMonkeyString.components(separatedBy: " ")
for word in words {
...
//desired output = "$5.67"
let penniesString = String(numOfPennies)
let cents  = penniesString.suffix(2)
let dollar  = penniesString.prefix(penniesString.count-2)
let dollarString = "$\(dollar).\(cents)"
print(dollarString)

let centsIndex = penniesString.index(penniesString.endIndex, offsetBy: -2);
let centsString = penniesString[centsIndex...];
let dollorString2 = penniesString[..<centsIndex]
print("$\(dollorString2).\(centsString)")
//: # Let or Var?
