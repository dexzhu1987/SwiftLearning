...
class Teenager: Babysitter {
    ...
    func playCandyland(_ numberOfTimes: Int){
        print("play Candyland \(numberOfTimes) times")
    }
...
var t1 = Teenager(age: 16, responsible: true, patience: Level.low)
t1.playCandyland(3)
print("Did the baby fall asleep \(t1.read("Snow White", firstLine: "Long long time ago", asleep: true))")
...
class Animal: Adorable {
    ...
    func frolick() {
        print("playing with \(species)")
    }
    
    func curlIntoSmallBall() {
        if (softFur && (self.size == Size.tiny || self.size == Size.small)) {
            print("It can curl into a small ball")
        } else {
            print("It can not curl into a small ball")
        }
    }
...
var a = Animal(species: "cat", numberOfLegs: 4, size: Size.small, softFur: true)
a.frolick()
a.curlIntoSmallBall()
//: __Problem 3__
//:
...
extension UIColor
{
    ...
    class func pistachio() {
        self.init(redValue: 147, greenValue: 197, blueValue: 114)
    }
}

var c = UIColor.pistachio()
print(c)
